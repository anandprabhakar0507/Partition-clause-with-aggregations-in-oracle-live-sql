REM   Script: Analytics - Partition clause with aggregation
REM   SQL from the KISS (Keep It Simply SQL) Analytic video series by Developer Advocate Connor McDonald. This script is the first look at the partition clause with aggregates.
...
select deptno, job, ename, sal,
           ...
           sum(sal) over 
               ( partition by deptno) sal_by_dept,
           sum(sal) over 
               ( partition by deptno, job) sal_by_dept_job
    from emp
    order by deptno, job;
